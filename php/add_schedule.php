<?php
// File: SIGARA/php/add_schedule.php
session_start();
include 'db.php'; // Pastikan path ini benar
header('Content-Type: application/json');

// Pastikan hanya admin yang bisa mengakses
if (!isset($_SESSION['role_user']) || $_SESSION['role_user'] !== 'Admin') {
    http_response_code(403); // Forbidden
    echo json_encode(['status' => 'error', 'message' => 'Akses ditolak.']);
    exit();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $kelas = $_POST['kelas'] ?? '';
    $hari = $_POST['hari'] ?? '';
    $mata_kuliah = $_POST['mataKuliah'] ?? ''; // Nama dari JS
    $waktu = $_POST['waktu'] ?? '';
    $ruang = $_POST['ruang'] ?? '';
    $nama_dosen = $_POST['dosen'] ?? ''; // Nama dari JS (dosen -> nama_dosen di DB)

    if (empty($kelas) || empty($hari) || empty($mata_kuliah) || empty($waktu) || empty($ruang) || empty($nama_dosen)) {
        echo json_encode(['status' => 'error', 'message' => 'Semua kolom wajib diisi.']);
        exit();
    }

    // Pastikan nama kolom sesuai dengan tabel `jadwal` Anda
    $stmt = $conn->prepare("INSERT INTO jadwal (kelas, hari, mata_kuliah, waktu, ruang, nama_dosen) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $kelas, $hari, $mata_kuliah, $waktu, $ruang, $nama_dosen);

    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'message' => 'Jadwal berhasil ditambahkan.']);
    } else {
        error_log("Error adding schedule: " . $stmt->error);
        echo json_encode(['status' => 'error', 'message' => 'Gagal menambahkan jadwal: ' . $stmt->error]);
    }
    $stmt->close();
    $conn->close();
} else {
    echo json_encode(['status' => 'error', 'message' => 'Metode request tidak diizinkan.']);
}
?>